# CostPilot: The Enterprise AI Control, Optimization, and Accountability Platform

**Complete NotebookLM source document — July 2026**

## Document purpose

This document is source material for explaining CostPilot in a podcast, briefing, presentation, or long-form conversation. It covers the complete product vision, the functionality represented in the current application, the working Salesforce and ServiceNow proofs, the major user journeys, the technical flow, real-world examples, product boundaries, implementation status, and future direction.
...
- **Proof of concept** means the capability exists in a working integration package or test path but is still being validated, secured, packaged, and hardened for customer use.
- **Planned direction** means the idea is part of the product strategy but should not be presented as generally available.

The current application is a pilot product. A working proof is not the same as a generally available, multi-tenant enterprise service.
...
- Salesforce OAuth metadata discovery and mapping recommendations are implemented.
- A Salesforce unlocked beta package has been built and validated with Apex test coverage, but it is not an AppExchange product.
- A live Salesforce Agentforce request has successfully traveled through CostPilot to a model and returned to Salesforce.
- ServiceNow OAuth connectivity, a governed Flow Designer action, record attribution, token and cost reporting, and response return have been demonstrated in a developer instance.
- HubSpot is represented in the universal connector architecture, but equivalent live OAuth discovery and governed execution remain planned work.

This distinction should be stated clearly.

---

## 17. Salesforce business context and Agentforce proof of concept

CostPilot includes Salesforce DX source and a built unlocked beta package. This supports a realistic customer-style installation test while remaining a pilot rather than an AppExchange release.
...
- Tabs.
- Permission sets.
- Apex Invocable Method actions that can be exposed to Agentforce or Flow.
- Apex tests.
- A connector permission set.
- Named Credential and External Credential metadata for authenticated callouts.
...
The Salesforce classes and actions are implemented and tested. Direct Apex diagnostics reach the live CostPilot endpoint. A real Agentforce-originated request has completed the full route from Salesforce through CostPilot to an LLM and back to Salesforce. Record IDs, visible record names, source type, source system, user identity, tokens, cost, model, and routing data can be captured.

An unlocked beta package, **CostPilot Salesforce Connector 0.1.0.1**, has been built with passing Apex tests and 84 percent package code coverage. A neutral **Business Record Assistant** test agent was also created so the package and action can be evaluated from the perspective of a new customer.

This remains a proof of concept under active validation. Customers still need to install the package, assign its permission set, configure credentials, and add the CostPilot action to the selected Agentforce agent or Flow. It should not be described as a finished AppExchange product or a zero-configuration installation.
...
## 30. Real-world scenario: ServiceNow governed AI work

An IT or operations user asks AI to summarize a ServiceNow incident or change request.
...
CostPilot checks risk, removes unnecessary ticket boilerplate, routes the task, executes the approved model call, and attributes the request to the incident, change request, or work order.

A working developer-instance proof uses a ServiceNow Flow Designer action named **CostPilot Governed AI Request**. The action accepts a prompt, record table, record system ID, task, agent name, and department. It returns the AI response, model tier, model name, routing decision, cost, input and output tokens, tokens pruned, work-item ID, and work-item name.

The proof successfully processed change request `CHG0030002`, returned a governed summary, and created a CostPilot business-context record linked by the permanent ServiceNow system ID. An older record-insert Business Rule was disabled so ordinary record creation would not be misreported as AI usage. This proves the governed action pattern, but it is not yet a packaged ServiceNow Store application.
...
- ServiceNow universal action contract and governed-request outputs.

### Proof of concept or active validation

- Salesforce Agentforce-to-CostPilot-to-model-to-Salesforce workflow has been demonstrated and is being hardened.
- Salesforce unlocked beta connector package installation and permission assignment.
- Salesforce workspace-key Named Credential and External Credential configuration.
...
- ServiceNow OAuth connection and Flow Designer governed-request action.
- ServiceNow record attribution and response return.
- Broad multi-user enterprise deployment.

### Planned direction

- Packaged ServiceNow Store installation and broader metadata discovery.
- Live HubSpot OAuth and metadata discovery.
- Partner template builder and connector marketplace.
- Salesforce AppExchange distribution and a guided post-install wizard.
