...
def account_profile(
    ...
    # request happened. Point-in-time attribution logic (Phase A2 of the
    # ROI/Business Impact investigation) lives in core.stage_attribution,
    # generalized beyond context_type == "opportunity" -- called here with
    # the same ("opportunity",) default this endpoint always used, so
    # output is unchanged; other context_types become reachable through the
    # same function for future callers.
    from core.stage_attribution import compute_stage_breakdown
    stage_breakdown = compute_stage_breakdown(db, account, work_item_ids, tx_base)
